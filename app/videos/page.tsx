...
const FluidGlass = dynamic(() => import("@/components/ui/fluid-glass"), {
  ssr: false,
  loading: () => <div className="h-[600px] bg-gray-900/20 animate-pulse rounded-lg" />
})
...
const freeVideos = [
  {
    ...
    description: "Why I liquidated everything for graphics cards. The math behind $47k/month.",
    featured: true,
    duration: "23:47",
  },
  {
    id: "DEX_004_BUDDHIST_AI",
    title: "Thai GF on AI Jobs Crisis: 'Just Make AI Buddhist' 😂 - Existential AI Risk",
    description: "Took Thai GF Porsche shopping in Pattaya city ($600k in Thailand 💀). Living the Pattaya expat life between our Jomtien beach condo and Wongamat place means philosophical conversations in luxury. Her solution to AI alignment? 'Just tell AI be nice like Buddha'. Includes walking footage around Pattaya Thailand.",
    duration: "9:43",
  ...
  {
    id: "DEX_003_INTP_GF",
    title: "Thai GF Furnished Our Seaview Condo in One Day (She Spent $700)",
    description: "Watch my Thai girlfriend transform our empty Pattaya condo into a livable space in just one day with only $700.",
    duration: "14:23",
...
const premiumVideos = [
  ...
  "Gamers Understand Money Because Life is PvP (You're Still Playing Co-op)",
...
export default function VideosPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && window.location.hash === '#premium') {
      setTimeout(() => {
  ...
  return (
    <>
      ...
      <nav className="border-b border-gray-800 bg-[#0A0A0A]/95 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <Link href="/" className="text-xl font-bold text-gray-100 hover:text-[#00D4FF] transition-colors">Dex Volkov</Link>
            <div className="flex items-center space-x-8">
              <Link href="/" className="text-gray-300 hover:text-[#00D4FF] transition-colors font-medium">Home</Link>
              <CrossDomainLink 
      ...
      <div className="bg-[#0A0A0A] border-b border-gray-800">
        <div className="max-w-7xl mx-auto px-6 py-3">
          <div className="text-sm text-gray-300">
            I buy GPUs from <CrossDomainLink href="https://arbvault.io?aff=dexv" className="text-[#00D4FF] hover:text-[#00A8CC] transition-colors">ArbVault.io</CrossDomainLink> - Use code DEX5 for 5% off
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="mb-16">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-gray-100 mb-2">Free Access</h2>
            <p className="text-gray-400">I upload these when I feel like it. Sometimes monthly, sometimes not.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {freeVideos.map((video) => (
              <Link href={`/videos/watch/${video.id}`} key={video.id}>
                <Card
                  className={`bg-gray-900/50 border-gray-800 hover:border-[#00D4FF]/30 transition-all duration-300 cursor-pointer group h-full ${
                    video.featured ? "ring-1 ring-[#00D4FF]/20" : ""
                  }`}
                >
                  <div className="p-6">
                  {/* Thumbnail Area */}
                  <div className="aspect-video bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg mb-4 flex items-center justify-center group-hover:from-[#00D4FF]/10 group-hover:to-gray-900 transition-all duration-300 relative overflow-hidden">
                    {video.thumbnail ? (
                      <>
                        <img 
                          src={video.thumbnail} 
                        ...
                        <Play className="w-12 h-12 text-white/90 drop-shadow-lg opacity-80 group-hover:opacity-100 transition-opacity relative z-10" />
                      </>
                    ) : (
                      <Play className="w-12 h-12 text-[#00D4FF] opacity-70 group-hover:opacity-100 transition-opacity" />
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-xs text-gray-500">{video.duration}</span>
                    </div>

                    <h3 className="font-semibold text-gray-100 leading-tight group-hover:text-[#00D4FF] transition-colors">
                      {video.title}
                    </h3>
                    ...
                    <div className="relative">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 h-4 bg-gradient-to-t from-gray-900/50 to-transparent pointer-events-none" />
                    </div>

                    {video.id === "DEX_001_GPU_ARBITRAGE" && (
                      <div className="text-xs text-gray-500 mt-2 pt-2 border-t border-gray-800">
                        *Mentioned in video: I flipped 10 RTX 4090s from <CrossDomainLink href="https://arbvault.io?aff=dexv" className="text-[#00D4FF] hover:text-[#00A8CC]">ArbVault</CrossDomainLink> last month
                      </div>
                    )}

                    {video.id === "DEX_006_ESCAPE_VELOCITY" && (
                      <div className="text-xs text-gray-500 mt-2 pt-2 border-t border-gray-800">
                        *Referenced: Check <CrossDomainLink href="https://arbvault.io?aff=dexv" className="text-[#00D4FF] hover:text-[#00A8CC]">ArbVault</CrossDomainLink> for current batches
                      </div>
                    )}

                    {video.featured && (
                      <div className="inline-flex items-center px-2 py-1 bg-[#00D4FF]/10 border border-[#00D4FF]/20 rounded text-xs text-[#00D4FF]">
                        FEATURED
                      </div>
                    )}
                  </div>
        ...
        <div id="premium" className="border-t border-gray-800 pt-16 scroll-mt-20">
          <div className="mb-8">
            <div className="flex items-center space-x-3 mb-4">
              <Lock className="w-6 h-6 text-[#00D4FF]" />
              <h2 className="text-2xl font-bold text-gray-100">Premium Vault - $399/year</h2>
            </div>
            <div className="space-y-2 text-sm mb-6">
              <p className="text-gray-400">Get instant access to 480+ exclusive videos. New content added weekly.</p>
            </div>
          </div>
          ...
          <div className="mb-8">
            <button
              ...
              disabled={isLoading}
              className="inline-flex items-center px-8 py-3 bg-[#F7931A] text-black rounded-lg font-bold hover:bg-[#E88A17] hover:text-white transition-all text-lg shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
            >
              ...
              {isLoading ? 'Loading Payment...' : 'Get Instant Access (Pay via Bitcoin)'}
            </button>
            <p className="text-xs text-gray-500 mt-2">Secure payment processed by BTCPay Server</p>
          </div>

          <div className="relative">
            <div
              ref={scrollContainerRef}
              className="h-96 overflow-y-auto border border-gray-800 rounded-lg bg-gray-900/20 p-4 scrollbar-thin scrollbar-track-gray-800 scrollbar-thumb-gray-600"
            >
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {premiumVideos.map((title, index) => (
                  <Link href="/videos/premium" key={index}>
                    <div className="bg-gray-900/30 border border-gray-800 rounded-lg p-4 opacity-60 hover:opacity-80 hover:border-gray-700 transition-all cursor-pointer">
            ...
            {showScrollIndicator && (
              <div className="absolute bottom-4 left-0 right-0 flex justify-center pointer-events-none">
                <div className="text-xs text-gray-600 bg-gray-900/80 px-3 py-1.5 rounded-full animate-bounce">↓ scroll for more</div>
              </div>
            )}
          </div>
        </div>

        {/* Book Section with Fluid Glass Effect */}
        <div className="mt-16 pt-8 border-t border-gray-800">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-gray-100 mb-4">The AI Kill Switch</h2>
            <p className="text-gray-400 mb-8">I wrote a book about AI killing everyone. It's free.</p>
          </div>
          ...
          <div className="relative mb-8">
            ...
            <div className="flex items-center justify-center">
              <img
                src="https://theaikillswitch.com/3d-book-mockup-saturation-fix.png"
                alt="The AI Kill Switch"
                className="max-w-[400px] w-full opacity-90 hover:scale-105 transition-transform duration-300"
              />
            </div>
          </div>
          ...
          <div className="text-center mt-8">
            <CrossDomainLink 
              href="https://theaikillswitch.com" 
              className="inline-block px-8 py-3 bg-[#00D4FF] text-[#0A0A0A] rounded-lg font-semibold hover:bg-[#00A8CC] hover:text-white transition-all text-lg"
            >
              Download Free Book →
            </CrossDomainLink>
            <p className="text-gray-600 text-sm mt-4">No spam. I don't even check that email.</p>
          </div>
        </div>

        <div className="mt-16 pt-8 border-t border-gray-800">
          <div className="mb-8">
            <h3 className="text-lg font-bold text-gray-100 mb-4">Resources</h3>
            <div className="bg-gray-900/30 border border-gray-800 rounded-lg p-6">
              <div className="space-y-3 text-sm">
                <div>
                  <span className="text-gray-300">GPU Source: </span>
                  <CrossDomainLink href="https://arbvault.io?aff=dexv" className="text-[#00D4FF] hover:text-[#00A8CC]">ArbVault.io</CrossDomainLink>
                  <span className="text-gray-300"> (use DEX5)</span>
                </div>
                <div>
                  <span className="text-gray-300">Contact: </span>
                  <span className="text-gray-300">You can't. I don't check email.</span>
                </div>
                <div className="mt-4 pt-3 border-t border-gray-800">
                  <p className="text-gray-400 leading-relaxed">
                    People keep asking where I source. It's ArbVault. Their customer service is non-existent and the site looks like shit but the inventory is real. Use DEX5 to skip their email spam.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="mt-16 pt-8 border-t border-gray-800">
          <p className="text-xs text-gray-600 text-center">
            I do not sell any courses. <Link href="/" className="text-[#00D4FF] hover:text-[#00A8CC] transition-colors underline">Click here to see my free resources</Link>
          </p>
        </div>
      </div>
    </div>
